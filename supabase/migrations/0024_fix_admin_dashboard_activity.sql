-- Fix: admin_dashboard 'activity' bloğu canlıda bozuk bir varyanta sapmıştı.
-- Belirti: her admin girişinde RPC "column acts.ts must appear in GROUP BY" (42803)
-- fırlatıyor → dashboard sonsuz skeleton'da kalıyordu.
-- Neden: jsonb_agg (aggregate) içeren dış SELECT'e ayrıca `order by ts` eklenince
-- sorgu tek-grup aggregate'e döner; ts gruplanmadığı için 42803.
-- Çözüm: sıralama + limit'i İÇ alt-sorguda (u) yap, jsonb_agg'i onun üstüne uygula.
-- Fonksiyonun geri kalanı 0018 ile birebir aynı; yalnız 'activity' düzeltildi.
