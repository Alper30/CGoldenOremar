// Türkiye il/ilçe verisi (81 il + ilçeleri) — adres formundaki il→ilçe bağlı
// seçimleri için. Statik/offline: harita API'si gerektirmez, maliyet yok.
// Anahtar = il adı; değer = o ilin ilçeleri (alfabetik). Bölge odağımız (Hakkâri,
// Şırnak, Van, Yüksekova/Başkale/Şemdinli ilçeleri) dahil tüm ülke kapsanır.
...
// Sıralı il listesi (dropdown için) — Türkçe alfabetik.
export const TR_PROVINCE_NAMES = Object.keys(TR_PROVINCES).sort((a, b) =>
  a.localeCompare(b, "tr"),
);

export function districtsOf(province: string): string[] {
  return TR_PROVINCES[province] ?? [];
}

// Telefon ülke kodu seçici için yaygın kodlar (Türkiye varsayılan). Marka
// kuralı gereği emoji/bayrak kullanılmaz — sade "+90 Türkiye" gösterilir.
